...
ini_t* ini_load(const char *filename) {
  ...
  CellFsErrno err = cellFsOpen(filename, CELL_FS_O_RDONLY, &fp, 0, 0);
  ...
  ini->data[sz] = '\0';
  ini->end = ini->data  + sz;
  err = cellFsRead(fp, ini->data, sz, &n);
  if (n != sz) {
    goto fail;
  }
...
const char* ini_get(ini_t *ini, const char *section, const char *key) {
  ...
  while (p < ini->end) {
    if (*p == '[') {
      ...
      /* Handle key */
      val = next(ini, p);
      if (!section || !strcmp(section, current_section)) {
        if (!strcmp(p, key)) {
          return val;
        }
      }
