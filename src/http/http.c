...
void HttpTransactionSendRequest(HttpTransaction *trans) {
    ...
    trans->err = cellHttpResponseGetStatusCode(trans->trans_id, &trans->status_code);
    if (trans->err < 0) {
        return;
    }
...
int HttpDownloadFile(HttpContext *ctx, HttpTransaction *trans, const char *path, void *hash_buf) {
    ...
    CellSha256WorkArea workarea;
    if (hash_buf) {
        cellSha256BlockInit(&workarea);
    }
    ...
    while (1) {
        ...
        if (hash_buf) {
            cellSha256BlockUpdate(&workarea, buffer, trans->recv);
        }
    ...
    if (hash_buf) {
        cellSha256BlockResult(&workarea, hash_buf);
    }
