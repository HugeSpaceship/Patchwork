.equ RESOURCE_TYPE_SCRIPT, 0xB # 0xB is the script resource type id

.global LBP1ScriptHook
LBP1ScriptHook:
    # r25 can probably be used to re-set r3, use r3 as our staging area
    lwz r3, 0(r3) # read pointer at r3 into r3
    lbz r3, 39(r3) # read the resource type from the structure

    cmpwi cr7, r3, RESOURCE_TYPE_SCRIPT 
    beq cr7, BailLBP1 # Branch to BailLBP1 if resource type is script

    # Fix registers and jump back
    clrldi r3, r25, 32 # Reset r3
    bla 0x7D1EC # AddToCSRQueue
    ba 0x81BCC # continue to original function
BailLBP1:
    ba 0x81B40 # Go to the error handling for when RNP isn't working

.global LBP2ScriptHook
LBP2ScriptHook:
    beq cr7, BailLBP2 # relocated branch from original function

    # we shouldn't need to care about r0, as anything that uses it sets it immediately
    lbz r0, 35(r29) # 35 bytes in is the resource type
    cmpwi cr7, r0, RESOURCE_TYPE_SCRIPT 
    beq cr7, BailLBP2 # Branch to BailLBP2 if resource type is script

    ba 0x153D78 # continue to original function
BailLBP2:
    ba 0x153D3C # Return 0 in resource check function

.global LBP3ScriptHook
LBP3ScriptHook:
    cmpwi r3, 0
    bne BailLBP3 # Fail resource load

    lbz r3, 0x23(r29) # 0x23 bytes in is the resource type
    cmpwi cr7, r3, RESOURCE_TYPE_SCRIPT
    beq cr7, BailLBP3 # Branch to BailLBP3 if resource type is script

    ba 0x1CFC78 # continue to original function
BailLBP3:
    ba 0x1CFD10

# Same as LBP3ScriptHook with some different jump offsets
.global LBP3JPScriptHook
LBP3JPScriptHook:
    cmpwi r3, 0
    bne BailLBP3JP

    lbz r3, 0x23(r29)
    cmpwi cr7, r3, RESOURCE_TYPE_SCRIPT
    beq cr7, BailLBP3JP
    
    ba 0x1CFB14
BailLBP3JP:
    ba 0x1CFBAC
